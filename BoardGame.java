...
public class BoardGame {
	...
	{	   
	    ...
	    initializeHallways();//rooms and hallways are separated for distribution
	...
	public void chooseAnAvatar()
	{
		...
		for(Player player:this.AllUsers)
		{		
	...
	public ArrayList<String> printOptionsMenu(ArrayList<Position>validMoves, Player user)
	{
		...
		Card suspect = user.getPlayerCharacter();
    	if(!user.getTurnHistory().contains("suggest")&&
    			!isInHallway(suspect.getX(),suspect.getY())
    			&& suspect.isMovedHere()){
    		options.add("suggest");
    	}
    ...
    public boolean disproveSuggestion(Player curr, Player next, Solution guess){
    	...
    	if(!disprovingCards.isEmpty()){
    		disproved = true;
    		next.displayPopUp(next.getPlayerCharacter().toString() 
    				+ ", which card would you like to show " 
    				+ curr.getPlayerCharacter().toString() 
    				+ " to disprove their suggestion:\n" 
    ...
    public void OrganizePlayerTurns(){//This function reorders AllUsers to be in character order.
    	...
    	for(int i=0; i<array.length; i++){
    		for(int j=0; j<this.AllUsers.size(); j++){
	    		if(this.AllUsers.get(j).getPlayerCharacter().toString().equals(array[i])){
		    		RearrangeOrder.add(this.AllUsers.get(j));
		    		break;
	    		}
    		}
    	}
    	this.AllUsers = RearrangeOrder;
    }
	...
	public int nextPlayerIndex(int index){
		int nextPlayer;
		nextPlayer = index%(AllUsers.size());
		if(nextPlayer == (AllUsers.size()-1)){
			nextPlayer = 0;
		}
		else{
			nextPlayer++;
    ...
    public void play()
	{
		...
		while(!gameOver)
		{
			for (int i=0; i<AllUsers.size();i++)
			{
				...
				if(user.getAlreadyAccusedASuspect()){
					continue;
				}
				user.setTurn(true);
				notifyAllPlayers("Its " + user.toString() + "'s ("+ user.getPlayerCharacter().toString()+ ")turn.");

				while (user.getTurn())
				{
			    	Solution guess = takeTurn(user);
			    	boolean disproved = true;
			    	//update character position;
			    	if(guess != null && !guess.getAccuse()){
			    		notifyAllPlayers(guess.toString());
			    		for(int j=0; j<AllUsers.size()-1;j++){//minus one is to not include guesser in disprover cycle
			    			Player next = AllUsers.get(nextPlayerIndex(i+j));
			    			
			    			disproved = disproveSuggestion(user,next,guess);
			    			if(disproved)	
			    				break;
			    		}
				    	if(!disproved&&guess.equals(answer)){
				    		notifyAllPlayers("Hmmmm. that is an interesting suggestion " + user.toString()+"\n");
				    	}
		    		}
					if(guess!=null && guess.getAccuse())
					{
						if(guess.equals(answer)){
							...
							user.setAlreadyAccusedASuspect(true);
							notifyAllPlayers(user.toString() + "(" +user.getPlayerCharacter().toString()+ ")" 
